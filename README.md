...
A Next.js web application that converts academic PDF papers into runnable Jupyter notebooks and opens them directly in Google Colab — powered by Groq's LLM API.
...
3. **Generate a notebook** — Groq's LLM converts the paper into a structured `.ipynb` Jupyter notebook
...
Built the end-to-end user flow: PDF upload form, PDF text extraction API route, Groq notebook generation API route, `.ipynb` file builder, anonymous Gist upload, and light/dark theme toggle with system preference detection.
...
# Fill in GROQ_API_KEY and GITHUB_TOKEN in .env.local
npm run dev
```
...
|---|---|---|
| `GROQ_API_KEY` | Yes | Groq API key for notebook generation |
| `GITHUB_TOKEN` | Yes | GitHub PAT with `gist` write scope |
...
app/
  api/
    ...
    generate/      # Groq notebook generation route
  page.tsx         # Main UI
